...
| queryParams     | additional query params (for small data only, see `answers` for large datasets)                                                                                  | no       | `[['name', 'Olivia'], ['age', '3']]`                       |
| answers         | pre-populate form answers via postMessage (recommended for large datasets to avoid CloudFront query string limits)                                               | no       | `{ name: 'Olivia', age: '3', preferences: [...] }`         |
